...
const Home = () => {
  ...
  <table>
    ...
    <tbody>
      {excelFiles &&
        Array.from(new Set(excelFiles.map((excelFile) => excelFile.eventName))).map((eventName) => {
          const matchingFiles = excelFiles.filter((file) => file.eventName === eventName);
          const latestFile = matchingFiles.reduce((prev, current) =>
            prev.createdAt > current.createdAt ? prev : current
          );
          const formattedDate = `${new Date(latestFile.createdAt).toLocaleDateString('en-US', {
            month: 'long',
            year: 'numeric',
            day: 'numeric',
          ...
          const maleCount = matchingFiles.filter((file) => file.sex === 'Male').length;
          const femaleCount = matchingFiles.filter((file) => file.sex === 'Female').length;
          const preferNotToSayCount = matchingFiles.filter((file) => file.sex === 'Prefer not to say').length;
